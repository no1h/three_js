...
function rysujPocisk(pocisk_group){
	let pocisk_geometry = new THREE.Geometry();
	let pocisk_material = new THREE.MeshBasicMaterial({color:0x00ff00});
	walec(pocisk_geometry, 10,1,0.3);
	let pocisk = new THREE.Mesh(pocisk_geometry,pocisk_material);


	pocisk.position.x = 2.5;
	pocisk.position.y = -0.5;
	pocisk.position.z = 3.8;
	pocisk.rotation.x = Math.PI/2;

	pocisk_group.add(pocisk);
}